...
import (
	...
	frame_utils "samples-go/grpc/experiment"
...
func main() {
	...
	grpcServer.Serve(fakeListener{actualListener: lis})
}
...
func (f fakeConn) Read(b []byte) (n int, err error) {
	...
	fmt.Println("------------------------------------------------FrameConfigs--------------------------------------------from: CLIENT")

	for {
		n := frame_utils.HeaderFields(framer)
		if n != 1 {
			break
		}
	}
	fmt.Println("------------------------------------------------FrameConfigs--------------------------------------------")
...
func (f fakeConn) Write(b []byte) (n int, err error) {
	...
	buf := bytes.NewBuffer(b)
	framer := http2.NewFramer(buf, buf)
	fmt.Println("------------------------------------------------FrameConfigs--------------------------------------------from: SERVER")
